...
function IndexCtrl($scope, $location, $rootScope, $http) {
    ...
    $scope.hiddenBlog = function (articalId) {
        $http.put('/api/post/' + articalId + '/hidden').success(function (data) {
            $scope.showOrHide =
            $location.path('/');
        })
    }
...
function SignupCtrl($scope, $http, $location) {
    ...
    $scope.form = {};
    $scope.submitPost = function () {
        $http.post('/signup/post', $scope.form).success(function (response) {
            if (response ) {
                currentUser = response.user.username;
                $scope.currentUser.username = currentUser;
                $location.path('/');
...
function ReadPostCtrl($scope, $rootScope, $http, $location, $routeParams) {
    ...
    $http.get('/api/post/' + $routeParams.id + "/comment").success(function (data) {
        console.log("data", data);
        $scope.comments = data.comments;
    })
    $scope.deleteComment = function (commentId) {
        $http.delete('/api/post/' + $routeParams.id + "/comment/" + commentId + '/delete').success(function (data) {
            $location.url('/');
        })
    }
    ...
    $scope.editCommentPost = function () {
        $http.put('/api/post/' + $routeParams.id + '/comment/' + $scope.editCommentId, $scope.form).success(function (data) {
            $location.url('/');
        });
    };
    ...
    $scope.commentPost = function () {
        ...
        $http.post('/api/post/' + $routeParams.id + '/comment', $scope.form).success(function (data) {
            console.log(data);
